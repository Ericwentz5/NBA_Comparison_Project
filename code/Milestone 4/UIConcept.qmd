...
server <- function(input, output) {
  ...
  output$overlapArea <- renderText({
    ...
    # Calculate the area for each individual polygon
    user_area <- polygon_area(user_values)
    nba_area <- polygon_area(nba_values)
    
    # Calculate overlap polygon area by taking the minimum of each attribute
    overlap_values <- pmin(user_values, nba_values)
    overlap_area <- polygon_area(overlap_values)
    
    # Calculate the overlap percentage based on the user and NBA areas combined
    overlap_percentage <- (overlap_area / polygon_area(rep(50, 4))) * 100
